...
const Timeline = (props: { jobs: Job[] }) => {
    ...
    const [transitions, api] = useTransition(slide[0], () => ({
        from: { opacity: 0, x: slide[1] * 200 },
        enter: { opacity: 1, x: 0 },
        leave: { opacity: 0, x: -1 * slide[1] * 200 },
    }));
    ...
    const prevSlide = () => setSlide(slide => [(((slide[0] - 1) % props.jobs.length) + props.jobs.length) % props.jobs.length, -1]);

    const nextSlide = () => setSlide(slide => [(slide[0] + 1) % props.jobs.length, 1]);
    ...
    return <div className="relative flex-col slideshow justify-end items-center hidden sm:flex">
        ...
        <div className="flex flex-row gap-x-4 mb-4">
            {props.jobs.map((j, idx) => <button key={j.name} onClick={() => setSlide([idx, 0])} className={`h-2 w-2 rounded-full ${slide[0] === idx ? "accent-color-bg" : "text-color-bg"}`} />)}
        </div>
    </div>
}
